export const BACKGROUND_SHADERS = {
  vertex: `
    ...
    void main() {
      ...
      vec2 noiseCoord = uv * vec2(6.0, 8.0); // Increased frequency for more dense noise
      float tilt = -0.8 * uv.y;
      ...
      for (int i = 0; i < 3; i++) {
        float noiseFlow = 10.0 + float(i) * 0.3; // Increased noise flow speed
        float noiseSpeed = 15.0 + float(i) * 0.3; // Increased noise speed
